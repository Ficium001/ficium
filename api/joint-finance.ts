/**
 * api/joint-finance.ts
 * GET/POST /api/joint-finance
 *
 * Single entry point for all joint-request / couple browser-facing calls.
 * Consolidated into one file for the same reason as internal.ts: Vercel
 * Hobby's 12-function limit. When upgrading to Pro, each action can be
 * split back into its own file with zero logic changes.
 *
 * Routes by `action`:
 *   GET  ?action=invitation-preview&token=...   (public — no auth)
 *   GET  ?action=couple                          (auth required)
 *   POST { action: "invitation-create", ... }    (auth required)
 *   POST { action: "invitation-respond", ... }   (auth required)
 *   POST { action: "invitation-revoke", ... }    (auth required)
 *
...
export default async function handler(req: any, res: any): Promise<void> {
  try {
    ...
    console.error("[joint-finance] UNCAUGHT:", msg, e instanceof Error ? e.stack : "");
...
async function route(req: any, res: any): Promise<void> {
  if (req.method === "GET") {
    ...
    if (action === "invitation-preview") return invitationPreview(req, res);
    if (action === "couple") return getCouple(req, res);
    return Response.error(res, "Unknown or missing action", 400, "INVALID_ACTION");
  }

  if (req.method === "POST") {
    const { action, ...body } = (req.body ?? {}) as { action?: string } & Record<string, unknown>;
    if (action === "invitation-create") return invitationCreate(body, req, res);
    if (action === "invitation-respond") return invitationRespond(body, req, res);
    if (action === "invitation-revoke") return invitationRevoke(body, req, res);
    return Response.error(res, "Unknown or missing action", 400, "INVALID_ACTION");
  }
...
// ── GET ?action=invitation-preview ──────────────────────────────────────────
// Public — shown on the /invite/:token landing page before login/signup.
...
// ── GET ?action=couple ───────────────────────────────────────────────────
// Symmetric for either partner — see api/couple.ts history for full notes.

async function getCouple(req: any, res: any): Promise<void> {
  let userId: string;
  try {
...
async function invitationCreate(body: Record<string, unknown>, req: any, res: any): Promise<void> {
  let userId: string;
  try {
    const user = await requireUser(req);
    userId = user.id;
  ...
  if (rpcErr) {
    console.error("[joint-finance:invitation-create] rpc error:", rpcErr);
...
async function invitationRespond(body: Record<string, unknown>, req: any, res: any): Promise<void> {
  let userId: string;
  try {
    const user = await requireUser(req);
    userId = user.id;
...
async function invitationRevoke(body: Record<string, unknown>, req: any, res: any): Promise<void> {
  let userId: string;
  try {
    const user = await requireUser(req);
    userId = user.id;
