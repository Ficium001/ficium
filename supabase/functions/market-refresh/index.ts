// market-refresh v5 — slim build, no bundle bloat
...
async function fetchFx() {
  try {
    const r = await fetch("https://api.frankfurter.app/latest?from=USD&to=EUR,GBP,ZAR,MUR",
...
async function fetchBom() {
  try {
    const r = await fetch("https://www.bom.mu/statistics/key-rates",
      { headers: { "User-Agent": "Ficium/1.0" }, signal: AbortSignal.timeout(10000) });
    if (!r.ok) return null;
...
async function fetchSemdex(): Promise<number | null> {
  try {
    const r = await fetch("https://www.stockexchangeofmauritius.com/api/v1/market/indices",
      { signal: AbortSignal.timeout(8000) });
    if (!r.ok) return null;
...
async function getPrev(): Promise<Record<string,number>> {
  ...
  return Object.fromEntries((data ?? []).map(r => [r.ticker_id, parseFloat(r.value)]));
}
...
Rules: plain English for everyday Mauritians, grounded in the numbers, categories: "Interest Rates"|"Currency"|"Stock Market"|"Economy"|"Savings"|"Lending", related_ticker_id one of: repo_rate|usd_mur|eur_mur|gbp_mur|semdex|avg_deposit_rate|avg_lending_rate (or omit), headline <80 chars, plain_english <180 chars.
...
Deno.serve(async () => {
  ...
  if (fx) {
    ...
    const { error, count } = await sb.from("market_fx_rates").upsert(rows, { onConflict:"currency_code,bank_name", count:"exact" });
