...
const schema = z.object({
  ...
  preferredTermMonths: z.number({ message: "Enter a term in months" }).int("Must be a whole number").min(1, "Minimum 1 month").max(360, "Maximum 30 years (360 months)"),
  maxRate: z.number().min(0).max(100).optional().or(z.nan().transform(() => undefined)),
      ...
      <div className="mx-auto w-full max-w-[520px]">
        ...
        <Card>
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-5">
            <Field label="Product type" htmlFor="productType" error={errors.productType?.message}>
              <Select id="productType" defaultValue="" invalid={!!errors.productType} {...register("productType")}>
            ...
            <Field label="Amount (MUR)" htmlFor="amount" error={errors.amount?.message}>
              <Input id="amount" type="number" inputMode="numeric" placeholder="e.g. 500000"
                invalid={!!errors.amount} {...register("amount", { valueAsNumber: true })} />
            </Field>

            <Field label="Purpose" htmlFor="purpose" hint="A short description — banks see this, not your name" error={errors.purpose?.message}>
              <Input id="purpose" type="text" placeholder="e.g. Expand kitchen equipment"
                invalid={!!errors.purpose} {...register("purpose")} />
            </Field>

            <Field label="Preferred term (months)" htmlFor="preferredTermMonths" hint="36 for 3 years, 60 for 5 years, etc." error={errors.preferredTermMonths?.message}>
              <Input id="preferredTermMonths" type="number" inputMode="numeric" placeholder="36"
                invalid={!!errors.preferredTermMonths} {...register("preferredTermMonths", { valueAsNumber: true })} />
            </Field>

            <Field label="Max acceptable rate (% APR)" htmlFor="maxRate" optional hint="Banks won't bid above this. Leave blank if no preference." error={errors.maxRate?.message}>
              <Input id="maxRate" type="number" step="0.1" inputMode="decimal" placeholder="e.g. 12"
                invalid={!!errors.maxRate} {...register("maxRate", { valueAsNumber: true })} />
            </Field>

            <Field label="Decision deadline" htmlFor="decisionDeadline" optional hint="When you'd like to accept a bid by" error={errors.decisionDeadline?.message}>
              <Input id="decisionDeadline" type="date" invalid={!!errors.decisionDeadline} {...register("decisionDeadline")} />
            </Field>
