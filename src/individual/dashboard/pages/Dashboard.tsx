...
function FinancialToolsSection() {
  ...
  const fmt = (n: number) => `MUR ${new Intl.NumberFormat("en-IN", { maximumFractionDigits: 0 }).format(Math.round(n))}`;
  const fmtNum = (n: number) => new Intl.NumberFormat("en-IN", { maximumFractionDigits: 0 }).format(Math.round(n));
      ...
      <div className="bg-white rounded-[22px] border border-ink/[0.06] shadow-sm overflow-hidden">
        <div className="p-5 lg:grid lg:grid-cols-2 lg:gap-8">
          ...
          <div className="space-y-6 mb-6 lg:mb-0">
            <FTSlider
              label="Amount (MUR)"
              value={amount} min={50_000} max={5_000_000} step={1_000}
            ...
            <FTSlider
              label="Term (Years)"
              value={term} min={1} max={30} step={1}
              display={`${term} ${term === 1 ? "Year" : "Years"}`} onChange={setTerm}
            />
          ...
          {/* Result panel */}
          <div
            className="rounded-2xl p-5 sm:p-6 text-white flex flex-col justify-between"
...
function FTSlider({
  label, value, min, max, step, display, onChange, typeable,
}: {
