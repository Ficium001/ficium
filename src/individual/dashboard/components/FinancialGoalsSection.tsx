// =============================================================
// Ficium — FinancialGoalsSection (v2)
// Renamed: "Your Active Requests" — provider-neutral language
// "banks ready" → "providers ready"
// loanLabel → provider-neutral CTA copy
// =============================================================
...
const GOAL_STYLE: Record<GoalType, {
  ...
  mortgage:   { icon: Home,          imgFrom: "#c47b2b", imgTo: "#7a4a1e", barColor: "#2A1FE6", ctaLabel: "Find home loan"       },
  vehicle:    { icon: Car,           imgFrom: "#4b5563", imgTo: "#1f2937", barColor: "#2A1FE6", ctaLabel: "Find vehicle loan"     },
  personal:   { icon: User,          imgFrom: "#0ea5e9", imgTo: "#0369a1", barColor: "#2A1FE6", ctaLabel: "Find personal loan"    },
  investment: { icon: TrendingUp,    imgFrom: "#0f0c29", imgTo: "#2A1FE6", barColor: "#2A1FE6", ctaLabel: "Find investment"       },
  education:  { icon: GraduationCap, imgFrom: "#059669", imgTo: "#065f46", barColor: "#059669", ctaLabel: "Find education loan"   },
  business:   { icon: Briefcase,     imgFrom: "#7c3aed", imgTo: "#4c1d95", barColor: "#7c3aed", ctaLabel: "Find business loan"    },
  savings:    { icon: PiggyBank,     imgFrom: "#d97706", imgTo: "#92400e", barColor: "#d97706", ctaLabel: "Find deposit offer"    },
  other:      { icon: Plus,          imgFrom: "#6b7280", imgTo: "#374151", barColor: "#6b7280", ctaLabel: "Post a request"        },
};
      ...
      <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-2 scrollbar-hide lg:grid lg:grid-cols-5 lg:overflow-visible">
        {goals.map((g) => (
          <RequestCard
            key={g.id}
            goal={g}
            onFind={() => navigate(g.loanRoute)}
            onDetail={() => navigate(`/requests/${g.id}`)}
          />
        ))}
...
function RequestCard({ goal, onFind, onDetail }: {
  goal:     Goal;
  onFind:   () => void;
  onDetail: () => void;
}) {
    ...
    <div className="flex-shrink-0 w-[170px] sm:w-[200px] lg:w-auto bg-white rounded-[18px] border border-ink/[0.06] shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="px-3 sm:px-4 pt-3 pb-1 flex-1 flex flex-col gap-1">
        ...
        <div className="text-[10px] font-bold flex items-center gap-1 mt-0.5" style={{ color: "#2A1FE6" }}>
          <Building2 size={10} /> {goal.banksReady} providers ready
        </div>
      </div>
