...
import { useMyInstitution, useMyRole, useInstitutionUsers, useWebhooks, useProducts } from "../../hooks/useInstitution";
...
export default function InstitutionSettings() {
  ...
  const TABS = [
    ...
    { key: "sla",      label: "SLA config",icon: Clock    },
  ] as const;
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[900px] mx-auto">
      ...
      {tab === "profile"  && <ProfileTab institution={institution} />}
...
// ── Profile tab ───────────────────────────────────────────────
function ProfileTab({ institution }: { institution: Record<string, unknown> | undefined }) {
  if (!institution) return null;
  const rows = [
    ["Institution name",   institution.name],
    ["Legal name",         institution.legal_name],
    ["Type",               String(institution.institution_type).replace(/_/g," ")],
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card overflow-hidden">
      ...
      <div className="divide-y divide-ink/[0.05]">
        {rows.map(([k, v]) => (
          <div key={String(k)} className="flex items-center px-6 py-4">
            <div className="w-48 text-[13px] text-muted flex-shrink-0">{k}</div>
            <div className="text-[13px] font-medium text-ink capitalize">{String(v ?? "—")}</div>
          </div>
        ))}
      </div>
      {/* Modules */}
      <div className="px-6 py-4 border-t border-ink/[0.07]">
        ...
        <div className="flex gap-2 flex-wrap">
          {((institution.modules ?? []) as string[]).map(m => (
...
function TeamTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  const handleInvite = async () => {
    ...
    // Submits through maker-checker submit_for_approval RPC
    ...
    <div className="bg-white rounded-2xl shadow-card overflow-hidden">
      <div className="px-6 py-4 border-b border-ink/[0.07] flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-muted mt-0.5">All invitations require maker-checker approval.</p>
        </div>
        {isAdmin && (
      ...
      {inviteSuccess && (
        <div className="mx-6 mt-4 bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-[13px] text-green-700">
          ✓ Invitation submitted for maker-checker approval.
        </div>
      )}
        ...
        <div className="mx-6 mt-4 p-4 bg-cream rounded-xl border border-ink/[0.07] space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={handleInvite}
              className="bg-ficium text-white text-[12px] font-bold px-4 py-2 rounded-xl hover:bg-ficium-deep transition-colors">
              Submit for approval
            </button>
      ...
      {isLoading ? (
        <div className="flex justify-center py-10"><div className="w-6 h-6 border-2 border-ficium border-t-transparent rounded-full animate-spin" /></div>
      ) : (
        <div className="divide-y divide-ink/[0.05]">
          {users.map(u => (
            <div key={u.id} className="flex items-center justify-between px-6 py-4">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-ficium/10 flex items-center justify-center text-[12px] font-bold text-ficium">
...
function ApiKeysTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  const handleCreate = async () => {
    ...
    // Generate key locally — hash stored server-side via RPC
    const raw = "fk_" + Array.from(crypto.getRandomValues(new Uint8Array(24))).map(b => b.toString(16).padStart(2,"0")).join("");
    await institutionSupabase.rpc("submit_for_approval", {
  ...
  const copyKey = () => {
    if (created) { navigator.clipboard.writeText(created); setCopied(true); setTimeout(() => setCopied(false), 2000); }
  };
    ...
    <div className="bg-white rounded-2xl shadow-card overflow-hidden">
      <div className="px-6 py-4 border-b border-ink/[0.07] flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-muted mt-0.5">Keys for programmatic access. Shown once — store securely.</p>
        </div>
        {isAdmin && (
      ...
      {created && (
        <div className="mx-6 mt-4 bg-amber-50 border border-amber-200 rounded-xl p-4">
          ...
          <div className="flex items-center gap-2">
            <code className="flex-1 bg-white border border-amber-200 rounded-lg px-3 py-2 text-[12px] font-mono text-ink overflow-auto">
              {showKey ? created : "fk_" + "•".repeat(40)}
            </code>
            <button onClick={() => setShowKey(s => !s)} className="text-amber-600 hover:text-amber-800">
              {showKey ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            </button>
            <button onClick={copyKey} className="flex items-center gap-1 text-[12px] font-semibold text-amber-700 hover:text-amber-900">
      ...
      {showCreate && (
        <div className="mx-6 mt-4 p-4 bg-cream rounded-xl border border-ink/[0.07] space-y-3">
          ...
          <div className="text-[12px] text-muted bg-ficium/5 border border-ficium/15 rounded-xl p-3">
            Scopes: <code className="font-mono">bids:write requests:read</code>. Key creation requires maker-checker approval.
          </div>
          <div className="flex gap-2">
      ...
      <div className="px-6 py-12 text-center text-muted text-[13px]">
        No API keys visible — keys are shown once at creation and the raw value is never stored.
      </div>
    </div>
  );
}
  ...
  const handleSave = async (productCode: string, bidWindow: number, autoWithdraw: number) => {
    await institutionSupabase
      ...
      .upsert({
        ...
        bid_window_minutes:    bidWindow,
        auto_withdraw_minutes: autoWithdraw,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card overflow-hidden">
      <div className="px-6 py-4 border-b border-ink/[0.07]">
        ...
        <p className="text-[12px] text-muted mt-0.5">Set bid window and auto-withdrawal times per product.</p>
      </div>
    ...
    <div className="flex items-center gap-4 px-6 py-4">
      ...
      <div className="flex items-center gap-3">
        <div>
          ...
          <input type="number" value={bidWindow} onChange={e => setBidWindow(Number(e.target.value))}
            disabled={!isAdmin}
        ...
        <div>
          ...
          <input type="number" value={autoWithdraw} onChange={e => setAutoWithdraw(Number(e.target.value))}
            disabled={!isAdmin}
            className="w-24 border border-ink/[0.12] rounded-lg px-2 py-1.5 text-[13px] outline-none focus:border-ficium disabled:bg-ink/[0.02] disabled:text-muted" />
        </div>
        {isAdmin && (
