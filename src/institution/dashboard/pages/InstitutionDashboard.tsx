...
// Overview: live stats, pending approvals, recent bid activity,
// marketplace snapshot, quick actions.
// =============================================================
import { Link } from 'react-router-dom'
import {
  TrendingUp, Clock, CheckCircle, XCircle,
  ArrowRight, AlertTriangle, Zap, Store,
} from 'lucide-react'
...
export default function InstitutionDashboard() {
  ...
  const expiringBids   = pending.filter(p => {
    ...
    return diff < 4 * 60 * 60 * 1000 && diff > 0 // expiring in < 4h
  }).length
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Active bids"
          value={bidsLoading ? '—' : activeBids}
          icon={TrendingUp}
          color="purple"
          sub="awaiting client decision"
          href="/institution/bids"
        />
        <StatCard
        ...
        <StatCard
          label="Accepted bids"
          value={bidsLoading ? '—' : acceptedBids}
          icon={CheckCircle}
          color="green"
          sub="all time"
        />
      ...
      {/* ── Two-column: recent bids + live marketplace ──────── */}
                ...
                <div key={req.id} className="px-5 py-3 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    {modules.includes('marketplace') && (
                      <Link
                        to="/institution/marketplace"
                        className="text-[9px] text-blue-500 hover:text-blue-400 flex items-center gap-0.5 justify-end mt-0.5"
                      >
...
// ── Sub-components ─────────────────────────────────────────────
function StatCard({
  label, value, icon: Icon, color, sub, href, alert
}: {
  label: string
  value: number | string
