    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => refetch()} className="text-[10px] text-slate-500 hover:text-slate-300 border border-[#1e2d3d] px-3 py-1.5 rounded-lg transition-colors">
            Refresh
          </button>
        </div>
      </div>
      ...
      <div className="bg-[#0a1628] border border-[#1e3a5f] rounded-lg px-4 py-2.5 flex items-center gap-2.5 mb-5">
        ...
        <p className="text-[10px] text-slate-500">
          Bids require maker-checker approval. A second admin must confirm in{' '}
      ...
      <div className="flex items-center gap-2 mb-5 flex-wrap">
        ...
        <button onClick={() => setProductFilter('all')} className={`text-[10px] font-bold px-3 py-1.5 rounded-full border transition-colors ${productFilter === 'all' ? 'bg-[#0f1929] border-blue-500 text-blue-400' : 'border-[#1e2d3d] text-slate-500 hover:text-slate-300'}`}>
          All products
        </button>
        {productTypes.map(pt => {
          const product = products.find(p => p.code === pt)
          return (
            <button key={pt} onClick={() => setProductFilter(pt)} className={`text-[10px] font-bold px-3 py-1.5 rounded-full border transition-colors ${productFilter === pt ? 'bg-[#0f1929] border-blue-500 text-blue-400' : 'border-[#1e2d3d] text-slate-500 hover:text-slate-300'}`}>
              {product?.label ?? pt}
            </button>
          )
        })}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          <StoreIcon className="w-10 h-10 text-slate-800 mx-auto mb-3" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(req => (
            <RequestCard key={req.id} request={req} onBid={() => setSelectedRequest(req)} canBid={!!institution?.modules.includes('marketplace')} />
          ))}
        </div>
      )}

      {selectedRequest && (
        <BidModal request={selectedRequest} onClose={() => setSelectedRequest(null)} onSubmit={handleBidSubmit} isSubmitting={submitBid.isPending} error={submitBid.error?.message} />
      )}
    </div>
  )
}
  ...
  return (
    <div className="bg-[#0b0f18] border border-[#141b27] rounded-xl p-5 hover:border-[#1e2d3d] transition-all">
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="text-[10px] text-blue-400 font-bold uppercase tracking-widest mb-1">{request.family_label ?? 'Financial product'}</div>
          <div className="text-[13px] font-bold text-slate-100">{request.product_label ?? request.product_type}</div>
        </div>
        <span className="bg-[#052e16] text-green-400 text-[9px] font-bold px-2 py-0.5 rounded-full border border-[#166534]">OPEN</span>
      </div>
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-[#070a0f] rounded-lg p-3">
          ...
          <div className="text-[13px] font-bold text-slate-200 font-mono">{request.currency} {Number(request.amount).toLocaleString()}</div>
        </div>
        {request.term_months && (
          <div className="bg-[#070a0f] rounded-lg p-3">
            <div className="text-[9px] text-slate-600 uppercase tracking-widest mb-1">Term</div>
            <div className="text-[13px] font-bold text-slate-200 font-mono">{request.term_months} months</div>
          </div>
        )}
      </div>
      {request.purpose && <p className="text-[10px] text-slate-600 mb-4 line-clamp-2">{request.purpose}</p>}
      <div className="flex items-center justify-between">
        <div className={`flex items-center gap-1.5 text-[10px] ${isUrgent ? 'text-orange-400' : 'text-slate-600'}`}>
          <Clock className="w-3 h-3" /><span className="font-mono">{timeLeft}</span>
        </div>
        {canBid && (
          <button onClick={onBid} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg transition-colors">
            <Zap className="w-3 h-3" />Place bid
          </button>
        )}
      </div>
    </div>
...
function BidModal({ request, onClose, onSubmit, isSubmitting, error }: {
  request: MarketplaceRequest; onClose: () => void; onSubmit: (data: BidForm) => void; isSubmitting: boolean; error?: string
}) {
      ...
      <div className="bg-[#0b0f18] border border-[#1e2d3d] rounded-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex items-start justify-between p-5 border-b border-[#141b27]">
          <div>
            ...
            <p className="text-[10px] text-slate-600 mt-0.5">{request.product_label ?? request.product_type} · {request.currency} {Number(request.amount).toLocaleString()}{request.term_months ? ` · ${request.term_months}m` : ''}</p>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input {...register('rate', { valueAsNumber: true, setValueAs: v => parseFloat(v) / 100 })} type="number" step="0.01" placeholder="8.75" className="w-full bg-[#070a0f] border border-[#1e2d3d] text-slate-200 rounded-lg px-3 py-2 text-[12px] font-mono focus:border-blue-500 outline-none" />
            ...
            <div>
              ...
              <select {...register('rate_type')} className="w-full bg-[#070a0f] border border-[#1e2d3d] text-slate-200 rounded-lg px-3 py-2 text-[12px] font-mono focus:border-blue-500 outline-none">
          ...
          <div>
            ...
            <input {...register('amount_offered', { valueAsNumber: true })} type="number" className="w-full bg-[#070a0f] border border-[#1e2d3d] text-slate-200 rounded-lg px-3 py-2 text-[12px] font-mono focus:border-blue-500 outline-none" />
          ...
          <div>
            ...
            <input {...register('term_months', { valueAsNumber: true })} type="number" className="w-full bg-[#070a0f] border border-[#1e2d3d] text-slate-200 rounded-lg px-3 py-2 text-[12px] font-mono focus:border-blue-500 outline-none" />
          ...
          <div>
            ...
            <textarea {...register('notes')} rows={2} className="w-full bg-[#070a0f] border border-[#1e2d3d] text-slate-200 rounded-lg px-3 py-2 text-[11px] font-mono focus:border-blue-500 outline-none resize-none" placeholder="Any special conditions..." />
          </div>
          <div className="bg-[#0a1628] border border-[#1e3a5f] rounded-lg p-3 text-[10px] text-slate-500">
            ⚠ This bid will be queued for maker-checker approval before submission to the client.
          </div>
          {error && <div className="bg-[#1c0000] border border-red-900 rounded-lg p-3 text-[10px] text-red-400">{error}</div>}
          <div className="flex gap-3 pt-1">
            <button type="submit" disabled={isSubmitting} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-[11px] font-bold py-2.5 rounded-lg transition-colors">
              {isSubmitting ? <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Zap className="w-3.5 h-3.5" />}
              Submit for approval
            </button>
            <button type="button" onClick={onClose} className="px-4 text-[11px] text-slate-500 hover:text-slate-300 border border-[#1e2d3d] rounded-lg transition-colors">Cancel</button>
          </div>
        </form>
      </div>
    </div>
