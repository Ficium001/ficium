...
import {
  ...
  Briefcase, CreditCard, BarChart2, Banknote,
} from "lucide-react";
...
export function Nav({ scrollY }: { scrollY: number }) {
  ...
  return (
    <nav className={["fixed top-0 left-0 right-0 z-50 transition-all duration-300",
      scrolled ? "bg-cream/85 backdrop-blur-xl border-b border-ink/[0.06]" : "bg-transparent border-b border-transparent",
    ].join(" ")}>
        ...
        <div className="flex items-center gap-4 sm:gap-9">
          <div className="hidden lg:flex gap-7 text-[15px] font-medium">
            {[{ l: "Loans", h: "#products" },{ l: "Deposits", h: "#products" },{ l: "Business", h: "#products" },{ l: "How it works", h: "#how-it-works" }].map((it) => (
              <a key={it.l} href={it.h} className="text-ink/75 hover:text-ink no-underline transition-colors">{it.l}</a>
            ))}
          </div>
          <div className="flex gap-2 sm:gap-3">
            ...
            <Link to="/register" className="inline-flex items-center bg-ink text-cream px-4 sm:px-5 py-2.5 rounded-pill text-sm font-semibold no-underline">Get started</Link>
          </div>
        </div>
      </div>
    </nav>
      ...
      <div className="w-full h-full rounded-[32px] sm:rounded-[38px] p-[44px_22px_22px] overflow-hidden relative bg-gradient-to-br from-ficium to-ficium-deep">
        ...
        <div className="flex justify-between items-center mt-2 mb-5">
          ...
          <div className="bg-white/15 px-2.5 py-1 rounded-pill text-white text-[11px] font-semibold flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-mint" /> LIVE
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2.5">
          {scene.offers.map((o, i) => {
            ...
            return (
              <div key={`${sceneIndex}-${i}`} className={["rounded-2xl px-4 py-3.5 transition-all duration-500", isActive ? "bg-white scale-[1.02]" : "bg-white/[0.08] border border-white/10 scale-100"].join(" ")}>
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <div className={["font-display text-[22px] font-bold", isActive ? "text-ficium" : "text-white"].join(" ")}>{o.rate}</div>
                    <div className={["text-[10px]", isActive ? "text-muted" : "text-white/50"].join(" ")}>{scene.label === "Investments" ? "12mo" : "APR"}</div>
                  </div>
                </div>
                {isActive && <div className="inline-block mt-2.5 px-2.5 py-0.5 rounded-pill text-[10px] font-bold text-ink" style={{ background: o.color }}>⚡ {o.badge}</div>}
              </div>
            );
          })}
        </div>
...
export function Hero({ scene, sceneIndex, activeOffer }: { scene: Scene; sceneIndex: number; activeOffer: number }) {
  return (
      ...
      <div className="relative z-[2] max-w-[1280px] mx-auto px-5 sm:px-8 grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-10 lg:gap-14 items-center">
        <div>
          ...
          <h1 className="font-display font-bold text-ink m-0 text-[44px] leading-[0.95] sm:text-6xl md:text-7xl lg:text-[88px] xl:text-[96px]">
            Credit, savings,<br />
            <span className="text-ficium">investment</span> &amp;<br />
            <span className="relative inline-block">
              business
              <svg className="absolute left-0 -bottom-2 w-full" viewBox="0 0 400 14" preserveAspectRatio="none">
                <path d="M2 8 Q 100 2, 200 7 T 398 6" stroke="#FFD84D" strokeWidth="6" fill="none" strokeLinecap="round" />
              </svg>
            </span><br />
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-8 sm:mt-10">
            <Link to="/register" className="inline-flex items-center justify-center gap-2 bg-ficium text-white px-6 sm:px-7 py-4 rounded-pill text-base font-semibold no-underline shadow-ficium">
              Get my offers <ArrowRight size={18} />
            </Link>
            <a href="#how-it-works" className="inline-flex items-center justify-center bg-transparent text-ink px-6 py-4 rounded-pill text-base font-semibold border-[1.5px] border-ink/15 no-underline">
              See how it works
            </a>
          </div>
          <div className="flex gap-2 mt-8 sm:mt-10 items-center">
            {SCENES.map((s, i) => (
              <div key={s.label} className={["h-2 rounded-pill transition-[width] duration-400", i === sceneIndex ? "w-7 bg-ficium" : "w-2 bg-ink/15"].join(" ")} />
            ))}
        ...
        <div className="relative flex justify-center mt-6 lg:mt-0 animate-[float_6s_ease-in-out_infinite] motion-reduce:animate-none">
          ...
          <div className="absolute -top-2 -left-4 sm:-left-10 bg-white px-3 py-2.5 rounded-xl shadow-card flex items-center gap-2 animate-[slideIn_1s_0.4s_both]">
            <div className="w-8 h-8 rounded-full bg-mint grid place-items-center"><TrendingDown size={16} /></div>
            <div><div className="text-[11px] text-muted">Rate dropped</div><div className="text-[13px] font-bold">−0.4%</div></div>
          </div>
          <div className="absolute bottom-20 -right-4 sm:-right-12 bg-white px-3 py-2.5 rounded-xl shadow-card flex items-center gap-2 animate-[slideIn_1s_0.8s_both]">
            <div className="w-8 h-8 rounded-full bg-accent grid place-items-center"><Bell size={16} /></div>
...
export function HowItWorksSection() {
  ...
  return (
    <section id="how-it-works" className="bg-ink text-cream py-20 sm:py-28 lg:py-32 px-5 sm:px-8 relative overflow-hidden">
      ...
      <div ref={ref} className={["relative max-w-[1280px] mx-auto transition-all duration-700", inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-7"].join(" ")}>
        <div className="mb-12 sm:mb-16 lg:mb-20 max-w-[720px]">
          <div className="text-xs sm:text-[13px] font-bold tracking-[0.12em] uppercase text-accent mb-3 sm:mb-4">How it works</div>
          <h2 className="font-display text-4xl sm:text-5xl lg:text-[56px] font-bold m-0">Three steps.<br />Zero friction.</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {[
            ...
            <div key={s.n} className="py-8 sm:py-10 border-t border-cream/15">
              <div className={["font-display text-base font-bold tracking-[0.08em] mb-6 sm:mb-8", s.c].join(" ")}>{s.n}</div>
              <h3 className="font-display text-2xl sm:text-3xl lg:text-[32px] font-bold m-0 mb-3 sm:mb-4">{s.t}</h3>
  ...
  return (
    <section id="products" className="py-20 sm:py-28 lg:py-32 px-5 sm:px-8 max-w-[1280px] mx-auto">
      <div className="mb-10 sm:mb-14 max-w-[720px]">
        ...
        <h2 className="font-display text-4xl sm:text-5xl lg:text-[56px] font-bold m-0">Four products. One marketplace.</h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6">
        <ProductCard icon={<HandCoins size={28} />} bg="bg-ficium" fg="text-white" tag="Personal"  title="Loans that compete for you"  desc="Personal loans, car loans, home loans. Banks across Mauritius bid against each other for your business." metric="Best rate seen"      metricValue="8.2% APR" />
        <ProductCard icon={<Building2 size={28} />} bg="bg-ink"    fg="text-cream" tag="Business"  title="SME funding, on demand"       desc="Working capital, equipment finance, growth credit. Banks compete to fund your business."                metric="Best rate seen"      metricValue="7.9% APR" />
        <ProductCard icon={<PiggyBank size={28} />} bg="bg-accent" fg="text-ink"   tag="Deposits"  title="Deposits with real yield"     desc="Lock in fixed-term deposits. Banks bid against each other to hold your money."                          metric="Top yield this week" metricValue="5.4% p.a." />
        <ProductCard icon={<LineChart size={28} />} bg="bg-mint"   fg="text-ink"   tag="Wealth"    title="Investments that find you"    desc="Bank wealth desks pitch their best portfolios for your profile. Compare returns, fees, terms."            metric="Avg. fee saving"    metricValue="0.4%" />
      </div>
...
function ProductCard({ icon, bg, fg, tag, title, desc, metric, metricValue }: {
    ...
    <div className={[bg, fg, "p-7 sm:p-9 rounded-[28px] min-h-[300px] sm:min-h-[320px] flex flex-col justify-between relative overflow-hidden transition-transform duration-400 hover:-translate-y-1.5"].join(" ")}>
      <div>
        <div className="flex justify-between items-start mb-7 sm:mb-8">
          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-white/[0.18] grid place-items-center">{icon}</div>
          <div className="bg-white/20 px-3 py-1 rounded-pill text-[11px] font-bold uppercase tracking-[0.06em]">{tag}</div>
        </div>
        <h3 className="font-display text-2xl sm:text-[28px] font-bold m-0 mb-3 leading-[1.1]">{title}</h3>
...
function SecondaryProductCard({ icon, label, desc, productType }: { icon: React.ReactNode; label: string; desc: string; productType: string }) {
  return (
    <Link to={`/register?product=${productType}`} className="group flex items-start gap-3 bg-white border border-ink/[0.08] hover:border-ficium/30 hover:shadow-sm rounded-2xl px-4 py-3.5 transition-all">
      <div className="w-8 h-8 rounded-xl bg-ficium/8 text-ficium flex items-center justify-center flex-shrink-0 group-hover:bg-ficium group-hover:text-white transition-colors">{icon}</div>
      <div className="min-w-0">
...
export function Trust() {
  ...
  return (
    <section className="py-12 sm:py-16 lg:py-20 pb-20 sm:pb-28 lg:pb-32 px-5 sm:px-8 max-w-[1280px] mx-auto">
      <div ref={ref} className={["grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-10 lg:gap-20 items-center transition-all duration-700", inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-7"].join(" ")}>
        <div>
          <div className="aspect-square bg-ficium rounded-[32px] p-8 sm:p-10 text-white relative overflow-hidden">
            ...
            <div className="font-display text-4xl sm:text-5xl font-bold mt-10 relative">Privacy<br />by design</div>
        ...
        <div>
          ...
          <h2 className="font-display text-4xl sm:text-5xl lg:text-[56px] font-bold m-0 mb-7 sm:mb-8">Your data, your terms.</h2>
    ...
    <section className="py-12 sm:py-16 px-5 sm:px-8 pb-20 sm:pb-28 lg:pb-32">
      <div className="max-w-[1280px] mx-auto bg-ficium rounded-[28px] sm:rounded-[40px] px-7 sm:px-12 lg:px-16 py-16 sm:py-24 lg:py-28 relative overflow-hidden text-white">
        ...
        <div className="relative max-w-[720px]">
          <h2 className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-[72px] xl:text-[88px] font-bold m-0 mb-5 sm:mb-6 leading-[0.95]">Banks compete. You choose.</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
            <Link to="/register" className="inline-flex items-center justify-center gap-2 bg-white text-ficium px-7 py-4 sm:py-5 rounded-pill text-base sm:text-[17px] font-bold no-underline">Start free <ArrowRight size={18} /></Link>
            <a href="mailto:kishan.jeebun@ficium.net" className="inline-flex items-center justify-center bg-transparent text-white px-6 py-4 sm:py-5 rounded-pill text-base sm:text-[17px] font-semibold border-[1.5px] border-white/30 no-underline">Contact us</a>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 sm:gap-10 pb-12 sm:pb-16 border-b border-cream/10">
          ...
          {[
            ...
            <div key={c.h}>
              ...
              <div className="flex flex-col gap-2.5">
                {c.l.map((item) => <a key={item} href="#" className="text-cream no-underline text-sm opacity-80 hover:opacity-100 transition-opacity">{item}</a>)}
              </div>
            </div>
          ))}
        </div>
