...
function StatTile({ icon, value, label, highlight }: {
  ...
  return (
    <Card padded={false} className={["p-4 flex flex-col gap-1.5 items-start", highlight ? "bg-ficium text-white border-ficium" : ""].join(" ")}>
...
function RequestCard({ request }: { request: RequestSummary }) {
  return (
    <Link to={`/requests/${request.id}`} className="no-underline">
      <Card padded={false} className="p-4 hover:border-ink/15 transition-colors cursor-pointer">
