...
function computeTotalIncome(d: DossierInput): number {
  return d.monthlyIncome + d.additionalIncome;
}
...
/**
 * Health Score (0-100) — overall financial health.
 * Combines income, assets, employment stability, debt load, credit history.
 */
export function computeHealthScore(d: DossierInput): number {
  let score = 50;

  const income = computeTotalIncome(d);
  if (income >= 200000) score += 22;
  else if (income >= 100000) score += 16;
  ...
  else if (dti > 0.25) score -= 8;
  else if (dti > 0.1) score -= 3;
...
/**
 * Risk Score (0-100) — higher = riskier borrower.
 * Inverse-ish of health, weighted differently for underwriting.
 */
export function computeRiskScore(d: DossierInput): number {
  let risk = 30;

  if (d.compliance.bankruptcy) risk += 35;
  if (d.compliance.blacklisted) risk += 30;
  if (d.compliance.missedRepayments) risk += 18;
  if (d.compliance.legalDisputes) risk += 12;
  if (d.compliance.isPep) risk += 10;

  const dti = computeDebtToIncome(d);
  if (dti > 0.6) risk += 25;
  else if (dti > 0.4) risk += 15;
  else if (dti > 0.25) risk += 8;

  if (d.employmentStatus === "unemployed") risk += 20;
  if (d.employmentStatus === "freelance" || d.employmentStatus === "student") risk += 8;

  const income = computeTotalIncome(d);
  if (income < 15000) risk += 15;
  else if (income < 30000) risk += 5;

  return Math.max(0, Math.min(100, Math.round(risk)));
}

/**
 * Affordability Score (0-100) — how comfortably this person can take on new debt.
 * Driven mainly by income vs existing obligations.
 */
export function computeAffordabilityScore(d: DossierInput): number {
  ...
  const dti = computeDebtToIncome(d);
  const disposableRatio = 1 - dti;

  let score = Math.round(disposableRatio * 80);

  // Buffer from assets adds confidence
  const netWorth = computeNetWorth(d.assets);
  if (netWorth > income * 12) score += 15;
  else if (netWorth > income * 6) score += 8;
  else if (netWorth > income * 3) score += 3;

  // Penalty for very low income regardless of ratios
...
export async function submitDossier(input: DossierInput): Promise<DossierResult> {
  ...
  // 1. financial_profiles (upsert)
  const { error: fpError } = await supabase.from("financial_profiles").upsert(
    {
      user_id: userId,
  ...
  // 2. employment_details (upsert) — store only relevant fields
  const e = input.employmentDetails;
  const { error: empError } = await supabase.from("employment_details").upsert(
    {
      user_id: userId,
      employer_name: e.employerName ?? null,
      industry: e.industry ?? null,
      job_title: e.jobTitle ?? null,
      years_of_employment: e.yearsOfEmployment ?? null,
      employment_type: e.employmentType ?? null,
      work_email: e.workEmail ?? null,
      employer_address: e.employerAddress ?? null,
      business_name: e.businessName ?? null,
      brn_number: e.brnNumber ?? null,
      years_in_business: e.yearsInBusiness ?? null,
      ...
      company_type: e.companyType ?? null,
      number_of_employees: e.numberOfEmployees ?? null,
      annual_revenue: e.annualRevenue ?? null,
      primary_profession: e.primaryProfession ?? null,
      primary_clients_region: e.primaryClientsRegion ?? null,
      portfolio_website: e.portfolioWebsite ?? null,
      pension_income: e.pensionIncome ?? null,
      other_income_sources: e.otherIncomeSources ?? null,
      institution_name: e.institutionName ?? null,
      sponsor_type: e.sponsorType ?? null,
      monthly_allowance: e.monthlyAllowance ?? null,
  ...
  const { error: assetError } = await supabase.from("asset_details").upsert(
    {
      ...
      business_assets: input.assets.businessAssets,
      other_assets: input.assets.otherAssets,
    },
    { onConflict: "user_id" }
  );
  if (assetError) return { ok: false, error: `Assets: ${assetError.message}` };

  // 4. loan_details — wipe + reinsert (simpler than diffing)
  await supabase.from("loan_details").delete().eq("user_id", userId);
  if (input.hasExistingLoans && input.loans.length > 0) {
    const { error: loanError } = await supabase.from("loan_details").insert(
      input.loans.map((l) => ({
        user_id: userId,
  ...
  // 5. compliance_details (upsert)
  const { error: compError } = await supabase.from("compliance_details").upsert(
    {
      user_id: userId,
      source_of_wealth: input.compliance.sourceOfWealth ?? null,
      source_of_wealth_other: input.compliance.sourceOfWealthOther ?? null,
      is_pep: input.compliance.isPep,
      pep_details: input.compliance.pepDetails ?? null,
      tax_residency: input.compliance.taxResidency,
      has_credit_issues:
        input.compliance.missedRepayments ||
        input.compliance.blacklisted ||
        input.compliance.bankruptcy ||
        input.compliance.legalDisputes,
      missed_repayments: input.compliance.missedRepayments,
      blacklisted: input.compliance.blacklisted,
      bankruptcy: input.compliance.bankruptcy,
